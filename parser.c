...
uint32_t encode_instruction(const char *line); // Combined encoding function
...
uint32_t encode_instruction(const char *line) {
...
int main() {
    ...
    assembly_file = fopen(FILENAME, "r");
    if (!assembly_file) {
    ...
    while (fgets(buffer, sizeof(buffer), assembly_file)) {
        char *line = trim_whitespace(buffer);
        if (strlen(line) == 0)
            continue;

        uint32_t encoded = encode_instruction(line); // Call the single function
        memory[instruction_index++] = encoded;
    ...
    printf("\nStarting Pipeline Execution from Clock Cycle 1:\n");
    run_pipeline(memory, instruction_index);
